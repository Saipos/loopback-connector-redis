...
exports.initialize = function initializeSchema(dataSource, callback) {
  console.log("fdsfksdfdsfdsmfkdslfmsdklfmdskfldsflm")
  if (!redis) return;
...
BridgeToRedis.prototype.save = function(model, data, callback) {
  ...
  this.client.hgetall(model + ':' + data.id, function(err, prevData) {
    ...
    this.client.hmset([model + ':' + data.id, data], function(err) {
      ...
      if(data && data.ttl) {
        console.log("tem ttl d", parseInt(data.ttl))
        console.log("model + ':' + data.id", model + ':' + data.id)
        this.client.expire([model + ':' + data.id, parseInt(data.ttl)], function(err) {
          console.log("err", err)
        });
      }
...
BridgeToRedis.prototype.addTTLIndexes = async function(key, ttl) {
  try {
...
async function(model, id, data, callback, prevData) {

  const createTTL = function() {
    try {
      if (keysToAddTTL.length) {
        for (const key of keysToAddTTL) {
          await this.client.expire([key, parseInt(data.ttl)]);
        }
      } 
    } catch(err) {
  ...
  Object.keys(i).forEach(function(key) {
    if (data.hasOwnProperty(key)) {
      ...
      if (data.ttl) {
        keysToAddTTL.push('i:' + model + ':' + key + ':' + val)
      }
    }
  ...
  console.log("keysToAddTTL", keysToAddTTL)
  if (schedule.length) {
...
BridgeToRedis.prototype.updateOrCreate = function(model, data, callback) {
  console.log("updateOrCreate", data)
  if (!data.id) {
